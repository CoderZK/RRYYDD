...
@interface XBJinRR_LoadViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation XBJinRR_LoadViewController
...
- (void )loadRecommendsCorrect:(void(^)(BOOL isSuccess))callBack{
    //getposterWithItype
    WS(bself);
    [XBJinRRNetworkApiManager getposterWithItype:@"1" Block:^(id data) {
        //Itype 类别:1贷款商品 2信用卡商品
//        [bself.tableView.mj_header endRefreshing];
        callBack(YES);
        MyLog(@"贷款返回的数据0：%@",data);
        if (rusultIsCorrect) {
            [bself.recommendLists removeAllObjects];
            NSArray *modelArr = [XBJinRR_RecommendsLoadModel mj_objectArrayWithKeyValuesArray:data[@"data"]];
            for (XBJinRR_RecommendsLoadModel *model in modelArr) {
                if (self.isBig == YES) {
                    if ([model.fenlei intValue] == 1) {
                        [bself.recommendLists addObject:model];
                    }
                }else{
                    if ([model.fenlei intValue] == 2) {
                        [bself.recommendLists addObject:model];
                    }
                }
            }
...
- (void )loadListCorrect:(void(^)(BOOL isSuccess))callBack{
    WS(bself);
    NSDictionary *params = @{@"page":@(self.page),@"rows":@"6",@"Itype":@"1"};
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section==0) {
        if (self.recommendLists.count > 2) {
            return SIZE(160.5);
        }
        return SIZE(80);
