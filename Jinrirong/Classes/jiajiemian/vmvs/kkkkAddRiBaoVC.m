...
@interface kkkkAddRiBaoVC ()
@property (weak, nonatomic) IBOutlet UITextField *TF;
@property (weak, nonatomic) IBOutlet UITextView *TV;
@end

@implementation kkkkAddRiBaoVC
...
- (IBAction)add:(id)sender {
    ...
    NSString * sql = [NSString stringWithFormat:@"insert into kkkk_riBao (title,content,status,userId) values ('%@','%@','%d','%@')",self.TF.text,self.TV.text,0,[UDefault getObject:@"phone"]];
    
    if (isOpen) {
        BOOL insert = [db executeUpdate:sql];
        if (insert) {
            [SVProgressHUD showSuccessWithStatus:@"添加日报成功"];
