...
- (FMDatabase *)fd {
    ...
    NSString *filePath = [documentPath stringByAppendingPathComponent:@"RYD.db"];
    NSString *filePathTwo = [[NSBundle mainBundle] pathForResource:@"RYD" ofType:@"db"];
    NSFileManager * fileManger = [NSFileManager defaultManager];
    NSError * error;
    
    if ([fileManger fileExistsAtPath:filePath] == NO && filePathTwo != nil) {
...
//        }
    ...
    //实例化FMDataBase对象
    NSLog(@"---path -- 11 ---:%@",filePath);
    NSLog(@"---path --- 22--- :%@",filePathTwo);
